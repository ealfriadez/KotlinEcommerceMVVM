...
import com.elionet.ecommerceappmvvm.domain.model.ErrorResponse
...
object ResponseToRequest {
    ...
    fun <T> send(result: Response<T>): Resource<T>{
        return try {
            ...
            e.printStackTrace()
            Resource.Failure(e.message?: "HttException desconocido")
        }catch (e: IOException){
            e.printStackTrace()
            Resource.Failure("Verifique su conexion a internet")
        }catch (e: Exception){
            e.printStackTrace()
